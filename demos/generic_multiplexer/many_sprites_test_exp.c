...
{    
    ...
    {
        if (MULTIPLEX_DONE) {
			if(XX<LOW_Y_THRESHOLD)
			{
				SPRY[NUMSPRITES-2]=255;
			}
			else
			{
				SPRY[NUMSPRITES-2]=XX;
			}
			if(XX>255-LOW_Y_THRESHOLD)
			{
				SPRY[NUMSPRITES-1]=255;
			}
			else
			{
            ...
            for(i=0;i<NUMSPRITES-2;++i)
            {
				...
				check_y = 40+_SPRITE_SEPARATION_*(i&7)+(yValues[XX]>>1);
				
				if(check_y<LOW_Y_THRESHOLD)
				{
					SPRY[i]=255;
				}
				else
